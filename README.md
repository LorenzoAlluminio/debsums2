...
debsums3 is an extended version of the file integrity check tool 'debsums'. The major difference to debsums is the ability to verify the md5sums online. The online verification is based on the control file within the debian packages, debsums3 uses a partial download to minimize the required traffic. Verification by a third party at a remote location is possible as well. In case of heavy paranoia or when md5sums are missing for a file, full package download and file verification is possible.

The software can be downloaded here: http://files.dw-itsecurity.de/debsums3.zip
...
```
$ python3 debsums3.py --directory / --online --writedb
```
...
```
$ python3 debsums3.py --file /bin/bash
...
12345 md5sums read from the directory where debian stores the package information. The md5sum matches locally. There are differences to the debsums3 storage. The debsums storage has not been saved.
```
...
```
$ python3 debsums3.py --online --file /bin/bash
...
```
$ python debsums3.py --online --package bash
...
There are 55 files found within the package, none of them are in the debsums3 storage.
...
```
$ python3 debsums3.py --online-full --package bash
...
1 file on (my) system is changed, checking `debsums3.log`:
...
```
$ python3 debsums3.py --directory=/bin --online
...
debsums3 stays on the device where directory is located, it will not follow mount points.
...
```
$ python3 debsums3.py --directory=/bin --writedb
...
A md5sum is calculated for this file before and after the debsums3 run, you may store that value somewhere offline.
...
```
$ python3 debsums3.py --directory=/bin --writedb --online
...
debsums3 reads the stored information for this run, all files were checked before. The hashdb contains "md5_online" for each entry after the run, the checksum for `hashdb.json` is updated.
...
```
$ python3 debsums3.py --update --online
```

First check `debsums3.log` for any irregularity. If you are sure you want the new status stored, run again and write to storage.

```
$ python3 debsums3.py --update --online --writedb
```

`--update` is to be run after `apt-get update`. It finds removed, added and changed files as well as changed uris.
This command only makes sense on a fully crawled system (`debsums3.py --directory=/`), since `--update` does not work on local files, but compares the dpkg cache to the hashdb.
...
```
$ python3 debsums3.py --verify-online
...
If the number of mismatched md5sums is not null, check `debsums3.log` for details.

This command is for everybody with a high paranoia level suspecting a deeply compromized system, therefore the command can and should be run on a different system, you will need to transfer the hashdb. Be aware that the download of all control files takes some time, you can watch the progress with `tail -f debsums3.log`.
...
The result of an integrity check is printed as a single character. Detailed information is logged into `debsums3.log`.

* verified online against debian package: dot (`.`) / `trustlevel=4`
* verified locally against debian package: star (`*`) / `trustlevel=3`
* verified locally against debsums3 md5sum library, needs `--writedb` in a previous debsums3 run: dash (`-`) / `trustlevel=2`
* not verified, probably new or changed file: plus (`+`) / `trustlevel=1`
* verification failed, see debsums3.log for info/warning: exclamation mark (`!`) / `trustlevel=0`
